...
public sealed class PartialLoadCoordinatorTests
{
    ...
    public void Discard_RemovesBufferAndAllowsReopen()
    {
        ...
        coordinator.Enqueue(logId, [NewEvent(1)]);
        coordinator.Enqueue(logId, [NewEvent(2)]);
        dispatched.Clear();
        ...
        coordinator.Enqueue(logId, [NewEvent(3)]);
    ...
    public void DiscardAll_ClearsAllBuffers()
    {
        ...
        coordinator.Enqueue(first, [NewEvent(1)]);
        coordinator.Enqueue(second, [NewEvent(2)]);
        coordinator.Enqueue(first, [NewEvent(3)]);
        coordinator.Enqueue(second, [NewEvent(4)]);
        dispatched.Clear();
    ...
    public void Dispose_ThenEnqueueAndFlush_IsSilent()
    {
        ...
        coordinator.Dispose();
        coordinator.Enqueue(EventLogId.Create(), [NewEvent(1)]);
        coordinator.Flush();
    ...
    public void Enqueue_EmptyDelta_DispatchesNothing()
    {
        ...
        coordinator.Enqueue(EventLogId.Create(), []);
    ...
    public void MarkFinalized_DropsAlreadyBufferedDelta()
    {
        ...
        coordinator.Enqueue(logId, [NewEvent(1)]); // first-seen: flushes immediately and clears the buffer
        coordinator.Enqueue(logId, [NewEvent(2)]); // already seen: buffered, not yet flushed
        dispatched.Clear();
