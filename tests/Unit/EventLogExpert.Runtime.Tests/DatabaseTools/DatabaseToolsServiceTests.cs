...
public sealed class DatabaseToolsServiceTests
{
    ...
    private sealed class TestableDatabaseToolsService(IDatabaseToolsOperation operation) : IDatabaseToolsService
    {
        public Task<DatabaseToolsResult> ShowAsync(
            ...
            => InvokeOperation(operation, logSink, progress, cancellationToken);

        public Task<DatabaseToolsResult> CreateAsync(CreateDatabaseRequest request, IProgress<DatabaseToolsLogEntry> logSink, IProgress<DatabaseToolsProgress>? progress, CancellationToken cancellationToken)
            => InvokeOperation(operation, logSink, progress, cancellationToken);

        public Task<DatabaseToolsResult> MergeAsync(MergeDatabaseRequest request, IProgress<DatabaseToolsLogEntry> logSink, IProgress<DatabaseToolsProgress>? progress, CancellationToken cancellationToken)
            => InvokeOperation(operation, logSink, progress, cancellationToken);

        public Task<DatabaseToolsResult> DiffAsync(DiffDatabaseRequest request, IProgress<DatabaseToolsLogEntry> logSink, IProgress<DatabaseToolsProgress>? progress, CancellationToken cancellationToken)
            => InvokeOperation(operation, logSink, progress, cancellationToken);

        public Task<DatabaseToolsResult> UpgradeAsync(UpgradeDatabaseRequest request, IProgress<DatabaseToolsLogEntry> logSink, IProgress<DatabaseToolsProgress>? progress, CancellationToken cancellationToken)
            => InvokeOperation(operation, logSink, progress, cancellationToken);
        ...
        private static async Task<DatabaseToolsResult> InvokeOperation(
            ...
            CancellationToken cancellationToken)
        {
            ITraceLogger logger = new StreamingTraceLogger(logSink);
