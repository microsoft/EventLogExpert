...
public sealed class FilterDraft
{
    ...
    ///     <see cref="WouldLoseDataSwitchingTo" /> reports data loss. No-op when <paramref name="target" /> equals the current
    ///     <see cref="Mode" />.
    /// </summary>
    ...
    ///     current mode carries. Pure predicate so callers can decide whether to prompt for confirmation before mutating state
    ///     via <see cref="ApplyModeSwitch" />.
    /// </summary>
