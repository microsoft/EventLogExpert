...
public class MergeDatabaseCommand(ITraceLogger logger) : DbToolCommand(logger)
{
    ...
    internal void MergeDatabase(string source, string targetFile, bool overwriteProviders)
    {
        ...
        try
        {
            using var probe = new ProviderDbContext(targetFile, false, false, Logger);
        ...
        {
            Logger.Error($"Target database '{targetFile}' is at schema v{targetState.CurrentVersion} but v{DatabaseSchemaVersion.Current} is required. Run the 'upgrade' command first.");
