...
public class UpdateServiceTests
{
    ...
    {
        GitReleaseModel release = new()
        {
            RawChanges = MockChanges
        };

        string result = release.Changes;

        Assert.Equal(16,result.Split("\r\n").Length);
        Assert.Contains("Updated projects to .NET 8 and updated nuget packages to latest version\r\n", result);
