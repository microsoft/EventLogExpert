...
internal sealed class PublisherMetadataHandle : IDisposable
{
    private readonly EvtHandle _publisherMetadataHandle;

    private PublisherMetadataHandle(string providerName, string? metadataPath = null)
    {
    ...
    public string MessageFilePath =>
        Environment.ExpandEnvironmentVariables(
            GetPublisherMetadataProperty(EvtPublisherMetadataPropertyId.MessageFilePath));

    public string ParameterFilePath =>
        Environment.ExpandEnvironmentVariables(
            GetPublisherMetadataProperty(EvtPublisherMetadataPropertyId.ParameterFilePath));
    ...
    internal static PublisherMetadataHandle? Create(
        string providerName,
        ...
        ITraceLogger? logger = null)
    {
        if (metadataPath is { Count: > 0 })
        {
            ...
            {
                PublisherMetadataHandle localeMetadata = new(providerName, path) { IsLocaleMetadata = true };
        ...
        PublisherMetadataHandle metadata = new(providerName);
