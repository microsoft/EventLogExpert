...
// Public cross-assembly seam: build a legacy message-table ILazyMessageSource without exposing the internal LegacyMessageFileSource.
public static class LegacyMessageSourceFactory
{
    public static ILazyMessageSource? TryCreate(IReadOnlyList<string> files, string providerName, ITraceLogger? logger) =>
        LegacyMessageFileSource.TryCreate(files, providerName, logger);
}
