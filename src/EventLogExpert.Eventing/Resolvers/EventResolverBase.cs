...
public class EventResolverBase : IDisposable
{
    ...
    protected EventResolverBase(IEventResolverCache? cache = null, ITraceLogger? logger = null)
    {
        _cache = cache;
        _logger = logger;
        _matcher = new ModernEventMatcher(_templates, logger?.ForCategory(LogCategories.ResolutionModern));
        _taskKeywords = new TaskKeywordResolver(cache, logger?.ForCategory(LogCategories.ResolutionTasks));
        _descriptions = new DescriptionFormatter(_templates, cache,
            logger?.ForCategory(LogCategories.ResolutionDescription), TryGetSupplementalDetails);
    }
        ...
        EventModel? supplementalModernEvent)
    {
        ...
        return new(eventRecord.PathName, eventRecord.LogPathType)
        {
            ...
            Opcode = _taskKeywords.ResolveOpcodeName(eventRecord, details, modernEvent, supplemental, supplementalModernEvent),
