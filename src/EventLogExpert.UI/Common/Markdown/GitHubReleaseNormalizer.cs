...
internal static partial class GitHubReleaseNormalizer
{
    ...
    /// <summary>
    ///     Matches a <c>* </c> bullet whose content optionally begins with a commit identifier — either a 40-character
    ///     SHA or a Markdown link (e.g. <c>[abc1234](https://...)</c>) — and rewrites it as a clean <c>- description</c>
    ///     bullet. Bullets without a commit prefix are simply converted from <c>*</c> to <c>-</c> style for consistency.
    /// </summary>
    ...
    /// <summary>
    ///     Strips the structural HTML wrappers and inline emphasis tags that historically appeared in GitHub release
    ///     bodies. Inner text is preserved.
    /// </summary>
    [GeneratedRegex(@"</?(?:details|summary|b|i|em|strong)[^>]*>", RegexOptions.IgnoreCase)]
    private static partial Regex LegacyHtmlTagRegex();

    /// <summary>
    ///     Captures a <c>&lt;summary&gt;</c> block and promotes its inner text to a section heading so the collapsible
    ///     "See More" label is preserved as a visible divider in the rendered notes. Any nested HTML tags inside the captured
    ///     text are removed by <see cref="AnyHtmlTagRegex" /> before the heading is emitted, so even non-legacy markup (e.g.
    ///     <c>&lt;iframe&gt;</c>) cannot surface as literal text in the heading.
    /// </summary>
