...
///     keyboard interaction (Space to flip), focus-visible appearance, and forced-colors rendering match the rest of
///     the app. The visible label is wired into the inner switch via <c>aria-labelledby</c> chaining so SR users hear
///     both the upstream filter context and the visible label.
/// </summary>
public sealed partial class ToggleWithLabel : InputComponent<bool>
{
    ...
    /// <summary>Visible text rendered next to the toggle (e.g. "Exclude"). When null/empty, behaves like a bare <see cref="Toggle" />.</summary>
