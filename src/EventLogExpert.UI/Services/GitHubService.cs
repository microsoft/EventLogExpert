...
public class GitHubService : IGitHubService
{
    ...
    public async Task<IEnumerable<GitReleaseModel>> GetReleases()
    {
        ...
        {
            _traceLogger.Trace($"{nameof(GetReleases)} Attempt to retrieve {response.RequestMessage?.RequestUri} failed: {response.StatusCode}.");
            throw new Exception($"Failed to retrieve GitHub releases. StatusCode: {response.StatusCode}");
        }

        _traceLogger.Trace($"{nameof(GetReleases)} Attempt to retrieve {response.RequestMessage?.RequestUri} succeeded: {response.StatusCode}.");
        ...
        if (content is null)
        {
            _traceLogger.Trace($"{nameof(GetReleases)} Failed to deserialize response stream.");
